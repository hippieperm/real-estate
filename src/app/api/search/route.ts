...
export async function POST(request: NextRequest) {
  try {
    // Use admin client to bypass RLS for public searches
    ...
    if (sort_by === "price_deposit") {
      queryBuilder = queryBuilder.order("price_deposit", {
        ascending: sort_order === "asc",
      });
    } else if (sort_by === "price_monthly") {
      queryBuilder = queryBuilder.order("price_monthly", {
        ascending: sort_order === "asc",
      });
    } else if (sort_by === "pyeong_exclusive") {
      queryBuilder = queryBuilder.order("pyeong_exclusive", {
        ascending: sort_order === "asc",
      });
    } else {
    ...
    // Debug: Check if any listings have created_by = null
    if (data && data.length > 0) {
      const nullCreatedBy = data.filter(
        (item: any) => item.created_by === null
      );
      console.log("Listings with null created_by:", nullCreatedBy.length);
    }
